...
# Connect directly to our finalized architecture's pipeline engine
...
async def run_cli_ingestion(file_path: str):
    ...
    try:
        ...
        parsed_text_dump = pdf_processor.extract_clean_text(raw_bytes)

        # 2. 🌟 FIXED: Removed 'await' so it correctly handles the synchronous pipeline return
        report_output = ingest_flow_engine.ingest_pdf_document(
...
if __name__ == "__main__":
    ...
    asyncio.run(run_cli_ingestion(args.file))# filepath: app/ingest.py
import os
import sys
import argparse
import asyncio
